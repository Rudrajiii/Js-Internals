...
module.exports = {
    ...
    stringifyJSON_internals: function () {
        const outputDir = path.join(process.cwd(), 'custom-js-functions');
        const outputPath = path.join(outputDir, 'custom_stringifyJSON.js');
 ...
 */
`;
            const code = $stringifyJSON.toString();
            ...
            const fileName = path.basename(outputPath);
            console.log(
                chalk.yellow('⚠ Skipped: ') + chalk.white(fileName) + chalk.gray(' already exists.')
            );
        }
    }
};
